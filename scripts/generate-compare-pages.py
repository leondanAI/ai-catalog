...
(function() {{
  ...
  function starsHtml(avg) {{
    if (!avg) return '<span style="color:var(--text3);font-size:13px">No reviews yet</span>';
  ...
  function fetchTool(slug, lang) {{
    return fetch(SB+'/rest/v1/tools?slug=eq.'+slug+'&lang=eq.'+lang+'&select=slug,name,description,best_for,pros,cons,badge,url,domain,users',
  ...
  function render(a, b, rA, rB) {{
    var avgA = rA.length ? rA.reduce(function(s,c){{return s+c.rating;}},0)/rA.length : null;
    var avgB = rB.length ? rB.reduce(function(s,c){{return s+c.rating;}},0)/rB.length : null;
    ...
    var rows = [
      ['Overview',    esc(a.description||'—'),  esc(b.description||'—')],
      ['Pricing',     '<span class="badge badge-'+esc(a.badge||'')+'">'+badgeLabel(a.badge)+'</span>', '<span class="badge badge-'+esc(b.badge||'')+'">'+badgeLabel(b.badge)+'</span>'],
      ['Users',       esc(a.users||'—'),         esc(b.users||'—')],
      ['Best for',    esc(a.best_for||'—'),      esc(b.best_for||'—')],
      ['Rating',      ratingA,                   ratingB],
      ['Advantages',  listHtml(a.pros,'✅'),     listHtml(b.pros,'✅')],
      ['Disadvantages', listHtml(a.cons,'❌'),   listHtml(b.cons,'❌')],
      ['Website',     domA ? '<a href="'+esc(a.url)+'" target="_blank" rel="noopener" style="color:var(--accent)">'+esc(domA)+'</a>' : '—',
                      domB ? '<a href="'+esc(b.url)+'" target="_blank" rel="noopener" style="color:var(--accent)">'+esc(domB)+'</a>' : '—'],
      ['',            '<div style="display:flex;gap:8px;flex-wrap:wrap"><a class="tool-aff" href="'+esc(a.url||'#')+'" target="_blank" rel="noopener">Visit →</a><a class="tool-review" href="'+BASE+'/tools/'+esc(a.slug)+'.html">Full review</a></div>',
                      '<div style="display:flex;gap:8px;flex-wrap:wrap"><a class="tool-aff" href="'+esc(b.url||'#')+'" target="_blank" rel="noopener">Visit →</a><a class="tool-review" href="'+BASE+'/tools/'+esc(b.slug)+'.html">Full review</a></div>'],
    ];
  ...
  function load(lang) {{
    Promise.all([
      fetchTool(SLUG_A, lang), fetchTool(SLUG_B, lang),
      fetchRating(SLUG_A),     fetchRating(SLUG_B)
    ]).then(function(res) {{
      var toolsA = res[0], toolsB = res[1], rA = res[2], rB = res[3];
      if (!toolsA[0] || !toolsB[0]) {{
        if (lang !== 'en') {{ load('en'); return; }}
        document.getElementById('cmpResult').innerHTML = '<div class="cmp-loading">Content not found.</div>';
        return;
      }}
      render(toolsA[0], toolsB[0], rA, rB);
    }}).catch(function() {{
