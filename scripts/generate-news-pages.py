...
def fit_title(title, limit=TITLE_LIMIT, brand=BRAND):
    ...
    # информативна (заполняет хотя бы 75% строки выдачи). Иначе «Claude Opus 5»
    # съело бы 25 символов из 60 — обрезанный полный заголовок полезнее.
    # Порог именно высокий ещё и потому, что короткая «голова» — это обычно
    # непереводимое название продукта («Notion Developer Platform 3.5»), и тогда
    # все 8 языковых версий получают один и тот же английский заголовок.
    min_head = int(limit * 0.75)
    for sep in (' — ', ' – ', ' -- ', ': ', ' - ', ' | '):
        if sep in base:
